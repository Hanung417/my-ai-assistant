...
const SetupPage = () => {
  ...
  const getGptResponse = async (message: string) => {
    const systemPrompt = localStorage.getItem("systemPrompt") || "당신은 친근한 AI 비서입니다.";
  ...
  const saveSettings = async () => {
    ...
    try {
      await axios.post("http://localhost:8000/api/profile/update-prompt", {
        prompt: prompt, // user_id 제거
      });
      console.log("성향 프롬프트 저장 완료");
    } catch (err) {
      console.error("성향 저장 오류:", err);
    }
